...
def create_ticket_node(state: SupportState) -> SupportState:
    ...
    elif state.get("intent") == "critical_ticket":
        problem = f"Reporte critico del cliente: {state.get('message')}"
...
def response_node(state: SupportState) -> SupportState:
    ...
    elif intent == "critical_ticket":
        response = (
            "Por lo que describes, puede requerirse revision tecnica directa.\n\n"
            "Cree un ticket de soporte tecnico para que el equipo revise el caso."
        )
...
def route_after_classify(state: SupportState) -> str:
    ...
    if intent in ["failed_guidance", "administrative_ticket", "critical_ticket"]:
