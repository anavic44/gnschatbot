    ...
    <div class="phone">
        ...
        <div class="chat">
            <div class="bubble bot">
                ...
                Puedes escribirme tu número de ticket, tu número de contrato o describir tu problema.
                <br><br>
                Ejemplos:
                <br>• mi internet está muy lento
                <br>• tengo luz roja en el módem
                <br>• TCK390236
                <br>• quiero cambiar mi plan
            </div>

            <div class="quick-buttons">
                <button onclick="setQuick('Consultar ticket')">Consultar ticket</button>
                <button onclick="setQuick('Mi internet está lento')">Internet lento</button>
                <button onclick="setQuick('Tengo luz roja en el módem y no tengo internet')">Luz roja</button>
                <button onclick="setQuick('Ejecutar prueba de conexión')">Prueba conexión</button>
                <button onclick="setQuick('Quiero escalar a técnico')">Escalar técnico</button>
                <button onclick="setQuick('Quiero hablar con soporte')">Soporte</button>
            </div>
...
def extract_ticket_number(message):
    match = re.search(r"TCK\d+", str(message).upper())
...
def infer_option_from_message(message, has_ticket=False):
    ...
    if "escalar" in text or "técnico" in text or "tecnico" in text or "luz roja" in text or "sin internet" in text or "no tengo internet" in text:
        return "5"

    if "soporte" in text or "hablar" in text or "plan" in text or "pago" in text or "contraseña" in text or "password" in text:
        return "6"

    if has_ticket or "ticket" in text or "folio" in text:
        return "1"
...
def build_ticket_based_response(option, ticket):
    ...
    logger.info(
        f"WHATSAPP_FLOW | mode=ticket | ticket={ticket_number} | option={option} | category={category} | decision={diagnosis.get('decision')}"
    )
...
def build_ticketless_response(option, description):
    ...
    if option == "4":
        ...
        response_payload["ping_test"] = ping_result
        response_payload["ping_source"] = "destino externo de referencia porque no hay ticket ni IP técnica del cliente"
        ...
        conversation = (
            "Te recomiendo contactar soporte para validar tu cuenta o generar un ticket.\n"
            "Puedo orientarte mejor si me dices si es falla de internet, pago, cambio de plan o instalación."
        )
    ...
    logger.info(
        f"WHATSAPP_FLOW | mode=no_ticket | option={option} | decision={classification.get('decision')} | category={classification.get('category')}"
    )
...
def whatsapp():
    ...
    option = infer_option_from_message(message, has_ticket=bool(ticket_number))
...
def api_chatbot():
    ...
    ticket_number = extract_ticket_number(message)
    option = str(data.get("option") or infer_option_from_message(message, has_ticket=bool(ticket_number)))
